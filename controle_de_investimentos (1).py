...
class Application:
    ...
    def __frame_detalhe(self, filtro, one=False):
        ...
        if one:
            ...
            print(lucro, '─ em frame detalhe')
            if not lucro:
                # dessa forma, pois o SQL retorna uma tupla numa lista
                lucro = 0
                print('recebeu 0')
            else:
                lucro = lucro[0][0]
    ...
    def __frame_investimento(self):
        ...
        def on_selected(event):
            ...
            if self.filtrando.get() != 'TODOS':
                ...
                possui = Funcs.preco_medio(self.filtrando.get())
                valor = Funcs.preco_medio(self.filtrando.get())
                # evitando erro de range
                if len(valor) == 2:
                    valor = (0, 0, 0)
                #   Imprimindo dado no freme ao lado da combobox
                self.__labels_precoM_lucroP(preco_medio=preco_medio[0][0], possui=possui[1], valor=valor[2],
